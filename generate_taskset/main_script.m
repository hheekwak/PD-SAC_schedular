...
u_start = 0.5; u_finish = 0.9; unit = 0.1;
...
for u = 1 : M
   ...
   %     [Data_raw, util_M, util_m, valid] = Generate_Taskset(N_task, [10 1000], [1,1], U(u), -1); % T with range
        ...
        if valid == true
            ...
            % end
            for j = 1: ss(1, 1)
                task(j, :) = Taskset(j, Data(j,1), Data(j,2), Data(j,3), Data(j,4));
            end
            [~,ind] = sort([task.T]);
            task = task(ind);
...
ss = size(sets);
fileID = fopen('../data/chain_case16_1000.txt', 'w');
fileNum = fopen('../data/chain_case16_1000_num.txt', 'w');

for u = 1 : ss(1,1)
    for i = 1 : ss(1,2)
        data = sets(u, i).Data;
        index_column = (1:size(data, 1))';
        data = [data(:, 1:3), index_column, data(:, 4)];
        num = sets(u, i).num_tasks;
        max_util = sets(u, i).util_M;
        min_util = sets(u, i).util_m;
        fprintf(fileID, '%6.1f %8.4f %6.1f %4d %4d \r\n', data');
