...
% Type 2 Delayed Completion compuation, hyper period option is only activated on 2/25/2025
% Common task calculation is activated on March 2025
% All others are commented out, so please check for other options

save_mk = 0;    % save (m,k) of all tasks as a file

ts_size = 5; % size of a taskset (number of tasks in a taskset)
n_ts = 1;  % number of tasksets for each case (e.g. utilization, max T position...)

% manually set chain configuration (chain structure)
cn_st1 = [1 2 3]; 
cn_st2 = [4 2 5];   % second tasks are common task
%cn_st3 = [11 12 13 14 15];
%cn_st4 = [10 11 12];
%cn_st5 = [13 14 15];

% common task presence- yes: 1, no: 0
common_task = 1;

if common_task == 1
    % set common task array from chain configuration: row = chain order, col = task order
    hp_common = {[], [], [];    % common tasks with higher priority chains
                 [], [1], []};  % array of hp-chain-idx that is overlapped
    
    lp_common = [0 1 0          % existance of common tasks with lower priority chain- yes: 1, no: 0
                 0 0 0];        % last row 0 0 0 no lp chain
end
...
% read data
data1 = load('../data/chain_case18_1.txt');
...
% add BT, blocking time (T-C) for common task with higher priority chain 
% add empty column for I, upperbound of idle time of each task

data1.ED = data1.T;     % CHECK: under delayed complition, max ED (phasing delay) is T
data1.BT = data1.T - data1.C;     % For common task HP common task blocking time (OR under job-abort, max ED is T - C)
data1.I(:) = NaN;
...
% TYPE 1: Job-Abort, TYPE 2: Delayed-Completion
...
for i = 1: height(taskset)
    taskset(i) = {data1((i-1)*ts_size+1:i*ts_size,["ID" "T" "C" "ED" "I" "BT"])};
    ...
    hp_block_time = zeros(length(st_ts), length(st_ts)); % common task block time with higher priority chain
    lp_block_time = zeros(length(st_ts));
    ...
    for j = 1: length(st_ts)
        for k = 1: length(st_ts{j})     % st_ts{j} jth chain configuration e.g.[3 2 5 6 7 8 1]
            ...
            % maxDM_cn(i, j) = max(maxDM_cn(i, j), taskset{i}{(taskset{i}{:,"ID"} == st_ts{j}(k)), "C"}); % max deadline miss length for Type1 (Job abort handling)
        ...
        % block_times when common task exists: hp is used in WCRT, lp is used in e2e_ltc as WCET

        for hp = 1: j-1      % sum of block times w/higher priority chains  
            common_task_id = find(cellfun(@(x) ismember(hp,x), hp_common(j, :)));
            for co_task = 1:length(common_task_id)
                hp_block_time(hp, j) = hp_block_time(hp, j) + taskset{i}{(taskset{i}{:,"ID"} == st_ts{j}(common_task_id(co_task))), "BT"};
            end
        end

        if common_task ==1   % common task exists
            if lp_common (j,1) == 1     % if start task is ct, T(start) - C(last)
                lp_block_time(j) = taskset{i}{(taskset{i}{:,"ID"} == st_ts{j}(1)), "T"} - taskset{i}{(taskset{i}{:,"ID"} == st_ts{j}(length(st_ts{j}))), "C"};
            end
            for k = 2: length(st_ts{j}) 
                if lp_common(j,k) == 1
                    cur_block = taskset{i}{(taskset{i}{:,"ID"} == st_ts{j}(k)), "T"} - taskset{i}{(taskset{i}{:,"ID"} == st_ts{j}(k-1)), "C"};
                    if cur_block > 0        % if preceding task C < current task T, blocking time can be
                        lp_block_time(j) = lp_block_time(j) + cur_block;
                    end
                end
            end
        end
        % WCET
        e2e_ltc_wo_DM_ED(i, j) = sumC_cn(i, j) + cum_int_wo_dm(j) + lp_block_time(j);
...
fileID_h2 = fopen('../output/cn_e2eltc_case18_1_type2_hyp_test.txt', 'w');
